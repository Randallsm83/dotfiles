...
- `powershell-modules.txt` — Installed PowerShell module list
- `Documents/PowerShell/` — PS profiles + scripts
- `Obsidian/` — Obsidian vault
- `.config/` — chezmoi config, gh, docker, scoop config, rdock config, etc.
- `.local/` — cargo registry, mise installs, nvim data/state, lslib tools
...
- **Scheduled tasks**: FanControl, MSIAfterburner, OpenRGB, Rdock, UtilCacheCleaner, uwd2 (created programmatically — reference XMLs in `~/.config/windows/scheduled-tasks/`)
...
This handles: registry tweaks, power plan, USB/NIC PM, Windows features, scheduled tasks, ethernet NIC tuning, TCP/network stack, NVIDIA DRS profile, and uwd2 download.

Review `$Config` at the top of the script — adjust paths if any changed after reinstall.

**Still manual:**

- **Flow.Launcher Startup** — scoop installs it, just needs startup task
- **ISLC Startup** — now scoop-installed (`scoop install islc`), create logon task manually
- **UtilCacheCleaner** — script at `~/.local/utilities/UtilCacheCleaner.ps1` must exist first (was not backed up — recreate if needed)
...
- [ ] Flow Launcher works
- [ ] Scheduled tasks running (FanControl, OpenRGB, Rdock, etc.)
